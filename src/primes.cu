...
__global__ void miller_rabin_kernel(const uint32_t* input, bool* output, int iters) {
...
int main() {
    ...
    cudaMemcpy(d_input, nums.data(), n * sizeof(uint32_t), cudaMemcpyHostToDevice);

    // Launch a separate thread for each element in the array
    int threadsPerBlock = 256;
    int blocksPerGrid = (n + threadsPerBlock - 1) / threadsPerBlock;
    miller_rabin_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_input, d_output, iters);
